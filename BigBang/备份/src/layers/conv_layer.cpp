...
void ConvLayer<dtype>::UpdateParams_GPU(const dtype* bottom_data, const dtype* delta) {
	...
	for (int i = 0; i < nums_; ++i) {
		bigbang_gpu_gemm(false, true, top_channels_, bottom_unroll_row, bottom_unroll_column, static_cast<dtype>(1.0),
			delta + i*top_channels_*top_row_*top_column_, bottom_data + unroll_size*i, static_cast<dtype>(0.0),
			mutable_kernels_diff_data);
		bigbang_gpu_mmadd(kernel_diff_data, mutable_kernels_diff_data, kernel_size, kernel_diff_data);
	}
	dtype* kernel_data = kernels_->mutable_gpu_data();
	bigbang_gpu_minus(kernel_data, kernel_diff_data, kernel_size, alpha_ / nums_, kernel_data);
	//sc

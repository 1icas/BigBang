...
__global__ relu_forward(const int size, const dtype* bottom_data, dtype* top_data) {
	const int index = blockIdx.x * blockDim.x + threadIdx.x;
	if (index < size) {
		top_data[index] = bottom_data[i] > 0 ? bottom_data[i] : 0;
	}
}

template<typename dtype>
__global__ relu_backward(const int size, const dtype* bottom_data, const dtype* top_diff_data,
	dtype* bottom_diff_data) {
	const int index = blockIdx.x * blockDim.x + threadIdx.x;
	if (index < size) {
		bottom_diff_data[index] = bottom_data[i] > 0 ? top_diff_data[i] * bottom_data[i] : 0;
	}
}
